...
void run_tests_disciplina(void) {
    ...
    tpDisciplina d2 = criarDisciplina("DISC2", "Disciplina 2");
    ...
    disciplina_set_forced_return(99);
    test_suite_expect(&suite, "create_disciplina - exceção forçada", 99, create_disciplina(&d2));
    ...
    disciplina_set_forced_return(99);
    test_suite_expect(&suite, "read_disciplina - exceção forçada", 99, read_disciplina(d1.codigo, &leitura));
    ...
    disciplina_set_forced_return(99);
    test_suite_expect(&suite, "update_disciplina - exceção forçada", 99, update_disciplina(d1.codigo, &atualizacao));
    ...
    disciplina_set_forced_return(99);
    test_suite_expect(&suite, "delete_disciplina - exceção forçada", 99, delete_disciplina(d1.codigo));
